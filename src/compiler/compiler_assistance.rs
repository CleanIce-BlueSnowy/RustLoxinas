...
impl Compiler {
    ...
    pub fn convert_types(&mut self, from: &ValueType, to: &ValueType) {
        ...
        match (from, to) {
            (Char, Integer(to)) => self.convert_types(&Integer(UInt), &Integer(to.clone())),
            (Integer(from), Integer(to)) => match (from, to) {
                (Byte, Byte) | (Byte, SByte) | (SByte, Byte) | (SByte, SByte) => (),

                (Byte, Short) | (Byte, UShort) => self.write_code(OpZeroExtendByteToWord),
                ...
                (SByte, Short) | (SByte, UShort) => self.write_code(OpSignExtendByteToWord),
                ...
                (Short, Int) | (Short, UInt) => self.write_code(OpSignExtendWordToDword),
                ...
                (UShort, Int) | (UShort, UInt) => self.write_code(OpZeroExtendWordToDword),
                ...
                (Int, Long) | (Int, ULong) => self.write_code(OpSignExtendDwordToQword),
                ...
                (UInt, Long) | (UInt, ULong) => self.write_code(OpZeroExtendDwordToQword),
                ...
                (Long, ExtInt) | (Long, UExtInt) => self.write_code(OpSignExtendQwordToOword),

                (ULong, ExtInt) | (ULong, UExtInt) => self.write_code(OpZeroExtendQwordToOword),
            ...
            (Integer(from), ValueType::Float(to)) => match (from, to) {
                ...
                (Short, ValueFloatType::Float) => self.write_code(OpConvertSWordToFloat),
                (UShort, ValueFloatType::Float) => self.write_code(OpConvertUWordToFloat),
                ...
                (Long, ValueFloatType::Float) => self.write_code(OpConvertSQwordToFloat),
                (ULong, ValueFloatType::Float) => self.write_code(OpConvertUQwordToFloat),
                (ExtInt, ValueFloatType::Float) => self.write_code(OpConvertSOwordToFloat),
                (UExtInt, ValueFloatType::Float) => self.write_code(OpConvertUOwordToFloat),
                (Byte, Double) => {
                    self.write_code(OpZeroExtendByteToWord);
                ...
                (Short, Double) => self.write_code(OpConvertSWordToDouble),
                (UShort, Double) => self.write_code(OpConvertUWordToDouble),
                (Int, Double) => {
                    self.write_code(OpSignExtendDwordToQword);
                ...
                (Long, Double) => self.write_code(OpConvertSQwordToDouble),
                (ULong, Double) => self.write_code(OpConvertUQwordToDouble),
                (ExtInt, Double) => self.write_code(OpConvertSOwordToDouble),
                (UExtInt, Double) => self.write_code(OpConvertUOwordToDouble),
            },
            (ValueType::Float(from), Integer(to)) => match (from, to) {
                ...
                (ValueFloatType::Float, Short) => self.write_code(OpConvertFloatToSWord),
                (ValueFloatType::Float, UShort) => self.write_code(OpConvertFloatToUWord),
                ...
                (ValueFloatType::Float, Long) => self.write_code(OpConvertFloatToSQword),
                (ValueFloatType::Float, ULong) => self.write_code(OpConvertFloatToUQword),
                (ValueFloatType::Float, ExtInt) => self.write_code(OpConvertFloatToSOword),
                (ValueFloatType::Float, UExtInt) => self.write_code(OpConvertFloatToUOword),
                (Double, Byte) => {
                ...
                (Double, Short) => self.write_code(OpConvertDoubleToSWord),
                (Double, UShort) => self.write_code(OpConvertDoubleToUWord),
                (Double, Int) => {
                ...
                (Double, Long) => self.write_code(OpConvertDoubleToSQword),
                (Double, ULong) => self.write_code(OpConvertDoubleToUQword),
                (Double, ExtInt) => self.write_code(OpConvertDoubleToSOword),
                (Double, UExtInt) => self.write_code(OpConvertDoubleToUOword),
            },
            (ValueType::Float(from), ValueType::Float(to)) => match (from, to) {
                (ValueFloatType::Float, ValueFloatType::Float) | (Double, Double) => (),
                (ValueFloatType::Float, Double) => self.write_code(OpConvertFloatToDouble),
                (Double, ValueFloatType::Float) => self.write_code(OpConvertDoubleToFloat),
            },
            (Integer(from), Bool) => self.write_code(match from {
                Byte | SByte => OpConvertByteToBool,
                Short | UShort => OpConvertWordToBool,
                Int | UInt => OpConvertDwordToBool,
                Long | ULong => OpConvertQwordToBool,
                ExtInt | UExtInt => OpConvertOwordToBool,
            }),
            (ValueType::Float(from), Bool) => match from {
                ValueFloatType::Float => self.write_code(OpConvertDwordToBool),
                Double => self.write_code(OpConvertQwordToBool),
            },
            (Bool, Integer(_)) | (Bool, ValueType::Float(_)) => {
    ...
    pub fn integer_code(
        ...
        if let ValueType::Integer(res_type) = this_type {
            self.write_code(match res_type {
                Byte | SByte => byte,
                Short | UShort => word,
                Int | UInt => dword,
                Long | ULong => qword,
    ...
    /// 符号相关整型操作指令快捷函数
    #[inline]
    pub fn sign_integer_code(
        &mut self,
        this_type: &ValueType,
        signed_byte: Instruction,
        unsigned_byte: Instruction,
        signed_word: Instruction,
        unsigned_word: Instruction,
        signed_dword: Instruction,
        unsigned_dword: Instruction,
        signed_qword: Instruction,
        unsigned_qword: Instruction,
        signed_oword: Instruction,
        unsigned_oword: Instruction,
    ) {
        use crate::types::ValueIntegerType::*;
        if let ValueType::Integer(res_type) = this_type {
            self.write_code(match res_type {
