...
impl Visitor<String> for AstPrinter {
    fn visit_binary_expr(&mut self, expr: &Expr) -> String {
        if let Expr::Binary{left, operator, right} = expr {
            ...
            panic!("Invalid expr type: need Expr::Binary");
        }
    }
    ...
    fn visit_grouping_expr(&mut self, expr: &Expr) -> String {
        if let Expr::Grouping{expression} = expr {
            ...
            panic!("Invalid expr type: need Expr::Grouping");
        }
    }
    ...
    fn visit_literal_expr(&mut self, expr: &Expr) -> String {
        if let Expr::Literal{value} = expr {
            ...
            panic!("Invalid expr type: need Expr::Literal")
        }
    }
    ...
    fn visit_unary_expr(&mut self, expr: &Expr) -> String {
        if let Expr::Unary{operator, right} = expr {
            ...
            panic!("Invalid expr type: need Expr::Unary");
        }
    }
}
...
pub fn test() {
    ...
    let test1 = Expr::Binary{
        ...
        operator: Token::new(TokenType::Operator(TokenOperator::Plus), 0, 0, 0),
        right: Box::new(Expr::Binary{
            left: Box::new(Expr::Literal{ value: Data::Integer(DataInteger::Int(2)) }),
            operator: Token::new(TokenType::Operator(TokenOperator::Slash), 0, 0, 0),
            right: Box::new(Expr::Literal{ value: Data::Integer(DataInteger::Int(3)) }),
        }),
    };
    // (-23s + 10u ** 2e) / (12.0f - 13 ^ 16)
    let test2 = Expr::Binary{
        left: Box::new(Expr::Grouping{ expression: Box::new(Expr::Binary{
            left: Box::new(Expr::Unary{
                operator: Token::new(TokenType::Operator(TokenOperator::Minus), 0, 0, 0),
            ...
            right: Box::new(Expr::Binary{
                ...
                operator: Token::new(TokenType::Operator(TokenOperator::Power), 0, 0, 0),
                right: Box::new(Expr::Literal{ value: Data::Integer(DataInteger::ExtInt(2)) }),
            }),
        }), }),
        operator: Token::new(TokenType::Operator(TokenOperator::Slash), 0, 0, 0),
        right: Box::new(Expr::Grouping{ expression: Box::new(Expr::Binary{
            ...
            right: Box::new(Expr::Binary{
                ...
                operator: Token::new(TokenType::Operator(TokenOperator::Caret), 0, 0, 0),
