...
impl<'a> ExprVisitor<Result<(ExprResolveRes, LinkedList<u8>), Vec<CompileError>>> for FrontCompiler<'a> {
    fn visit_binary_expr(&mut self, _this: *const Expr, expr: &ExprBinary) -> Result<(ExprResolveRes, LinkedList<u8>), Vec<CompileError>> {
    ...
    fn visit_grouping_expr(&mut self, _this: *const Expr, expr: &ExprGrouping) -> Result<(ExprResolveRes, LinkedList<u8>), Vec<CompileError>> {
    ...
    fn visit_literal_expr(&mut self, _this: *const Expr, expr: &ExprLiteral) -> Result<(ExprResolveRes, LinkedList<u8>), Vec<CompileError>> {
    ...
    fn visit_unary_expr(&mut self, _this: *const Expr, expr: &ExprUnary) -> Result<(ExprResolveRes, LinkedList<u8>), Vec<CompileError>> {
        let (right_res, mut right_code) = expr.right.accept(self)?;
    ...
    fn visit_as_expr(&mut self, _this: *const Expr, expr: &ExprAs) -> Result<(ExprResolveRes, LinkedList<u8>), Vec<CompileError>> {
        let (inside_res, mut inside_code) = expr.expression.accept(self)?;
    ...
    fn visit_variable_expr(&mut self, _this: *const Expr, expr: &ExprVariable) -> Result<(ExprResolveRes, LinkedList<u8>), Vec<CompileError>> {
        let (expr_res, slot, is_ref) = Self::pack_error(self.resolver.resolve_variable_expr(expr))?;
        let expr_code = Self::pack_error(self.compiler.compile_variable_expr(&expr_res, slot, self.in_assign, self.in_ref_let, is_ref))?;
        return Ok((expr_res, expr_code));
    }
}
