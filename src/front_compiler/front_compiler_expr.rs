...
impl<'a> ExprVisitor<CompileResultList<(ExprResolveRes, Vec<u8>)>> for FrontCompiler<'a> {
    fn visit_binary_expr(&mut self, _this: *const Expr, expr: &ExprBinary) -> CompileResultList<(ExprResolveRes, Vec<u8>)> {
    ...
    fn visit_grouping_expr(&mut self, _this: *const Expr, expr: &ExprGrouping) -> CompileResultList<(ExprResolveRes, Vec<u8>)> {
    ...
    fn visit_literal_expr(&mut self, _this: *const Expr, expr: &ExprLiteral) -> CompileResultList<(ExprResolveRes, Vec<u8>)> {
    ...
    fn visit_unary_expr(&mut self, _this: *const Expr, expr: &ExprUnary) -> CompileResultList<(ExprResolveRes, Vec<u8>)> {
        let (right_res, mut right_code) = expr.right.accept(self)?;
    ...
    fn visit_as_expr(&mut self, _this: *const Expr, expr: &ExprAs) -> CompileResultList<(ExprResolveRes, Vec<u8>)> {
        let (inside_res, mut inside_code) = expr.expression.accept(self)?;
    ...
    fn visit_variable_expr(&mut self, _this: *const Expr, expr: &ExprVariable) -> CompileResultList<(ExprResolveRes, Vec<u8>)> {
