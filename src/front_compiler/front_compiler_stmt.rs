...
use crate::stmt::{Stmt, StmtAssign, StmtBlock, StmtBreak, StmtContinue, StmtEmpty, StmtExpr, StmtFor, StmtIf, StmtInit, StmtLet, StmtLoop, StmtPrint, StmtVisitor, StmtWhile};
...
impl<'a> StmtVisitor<CompileResultList<()>> for FrontCompiler<'a> {
    ...
    fn visit_assign_stmt(&mut self, _this: *const Stmt, stmt: &StmtAssign) -> CompileResultList<()> {
    ...
    fn visit_if_stmt(&mut self, _this: *const Stmt, stmt: &StmtIf) -> CompileResultList<()> {
        ...
        let else_if_expr: Vec<_> =
            stmt.else_if_branch.iter()
        ...
        let if_chunk = if let Stmt::Block(temp) = &stmt.if_branch.1 { temp } else { unreachable!("Not a block statement.") };
        ...
        for (condition_code, (_chunk_expr, chunk_block)) in branch_expr_codes.iter_mut().zip(stmt.else_if_branch.iter()) {
            ...
            let chunk = if let Stmt::Block(temp) = chunk_block { temp } else { unreachable!("Not a block statement.") };
    ...
    fn visit_loop_stmt(&mut self, _this: *const Stmt, stmt: &StmtLoop) -> CompileResultList<()> {
        ...
        let chunk = if let Stmt::Block(temp) = &stmt.chunk { temp } else { unreachable!("Not a block statement.") };
        self.compile_scope(&mut errors, &chunk.statements);
        ...
        for patch in &self.continue_patches {
            if patch.loop_tag == this_tag || matches!(patch.loop_tag, None) {
                ...
                self.codes[(patch.patch_pos as usize)..(patch.patch_pos as usize + 4)].copy_from_slice(&jump_start.to_le_bytes());
            }
        }
        self.continue_patches.retain(|patch| (patch.loop_tag != this_tag) && !matches!(patch.loop_tag, None));
    ...
    fn visit_continue_stmt(&mut self, _this: *const Stmt, stmt: &StmtContinue) -> CompileResultList<()> {
        // 直接在这里分析并编译
        if !self.context.in_loop {
            return Err(vec![CompileError::new(&stmt.pos, "Cannot use 'continue' outside a loop.".to_string())]);
        }
        ...
        self.continue_patches.push(ContinuePatch::new(stmt.tag.clone(), continue_location));

        return Ok(());
    }

    fn visit_print_stmt(&mut self, _this: *const Stmt, stmt: &StmtPrint) -> CompileResultList<()> {
        let (expr_res, expr_code, expr_pos) =
