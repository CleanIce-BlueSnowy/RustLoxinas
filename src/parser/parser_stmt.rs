...
use crate::stmt::{Stmt, StmtAssign, StmtBlock, StmtBreak, StmtContinue, StmtEmpty, StmtExpr, StmtFor, StmtIf, StmtInit, StmtLet, StmtLoop, StmtPrint, StmtWhile};
...
impl Parser {
    ...
    fn start_with_expr(&mut self) -> SyntaxResult<Stmt> {
        ...
        } else if parser_can_match!(self, Operator(PlusEqual | MinusEqual | StarEqual | SlashEqual | AndEqual | PipeEqual | CaretEqual | ModEqual)) {
    ...
    /// 语法糖脱糖——自赋值运算符
    fn desugar_self_assign_stmt(&mut self, expr: Expr, assign_operator: Rc<Token>) -> SyntaxResult<Stmt> {
        use crate::tokens::TokenType::*;
        use crate::tokens::TokenOperator::*;
        use crate::expr::{ExprVariable, ExprBinary};

        let (another_expr, left_pos) =
            if let Expr::Variable(var) = &expr {
                ...
                return Err(SyntaxError::new(&expr_get_pos!(expr), "Must be a left value.".to_string()));
            };

        let operator =
            Rc::new(Token::new(
                Operator(
                    match &assign_operator.token_type {
        ...
        return Ok(Stmt::Assign(Box::new(
            StmtAssign {
                pos: Position::new(left_pos.start_line, left_pos.start_idx, final_pos.end_line, final_pos.end_idx),
                assign_vars: vec![expr],
                right_expr: Expr::Binary(Box::new(
                    ExprBinary {
    ...
    fn let_stmt(&mut self) -> SyntaxResult<Stmt> {
        ...
        let (name, name_pos) =
            if let Identifier(temp) = &token.token_type {
                self.advance();
        ...
        let var_type =
            if parser_can_match!(self, Operator(Colon)) {
    ...
    fn init_stmt(&mut self) -> SyntaxResult<Stmt> {
        ...
        let name =
            if let Identifier(temp) = &name_token.token_type {
                temp
            } else {
    ...
    fn if_stmt(&mut self) -> SyntaxResult<Stmt> {
        ...
        return if parser_can_match!(self, Keyword(Else)) {
            if parser_can_match!(self, Keyword(If)) {
                ...
                let mut else_if = if let Stmt::If(temp) = else_if_stmt { temp } else { unreachable!() };
    ...
    fn loop_stmt(&mut self) -> SyntaxResult<Stmt> {
        ...
        let loop_pos = Position::new(keyword_loop.line, keyword_loop.start, keyword_loop.line, keyword_loop.end);

        // 处理标记
        let tag =
            if let Tag(tag_name) = &self.peek().token_type {
                self.advance();
                Some(tag_name.clone())
            } else {
                None
        ...
        let err_pos = Position::new(loop_pos.end_line, loop_pos.end_idx, loop_pos.end_line, loop_pos.end_line + 1);
        parser_consume!(self, Paren(LeftBrace), &err_pos, "Expect '{'.".to_string())?;

        let chunk = self.block_stmt()?;
        let final_pos = self.get_final_pos();

        return Ok(Stmt::Loop(Box::new(
            StmtLoop {
                pos: Position::new(loop_pos.start_line, loop_pos.start_idx, final_pos.end_line, final_pos.end_idx),
                chunk,
                tag,
            }
        )));
    ...
    fn for_stmt(&mut self) -> SyntaxResult<Stmt> {
        ...
        let for_pos = Position::new(keyword_for.line, keyword_for.start, keyword_for.line, keyword_for.end);
        ...
        let condition_pos = expr_get_pos!(&condition);
        let err_pos = Position::new(condition_pos.end_line, condition_pos.end_idx, condition_pos.end_line, condition_pos.end_idx + 1);
        parser_consume!(self, Operator(Semicolon), &err_pos, "Expect ';' after the expression.".to_string())?;
        ...
        if !matches!(update, Stmt::Expr(_) | Stmt::Assign(_) | Stmt::If(_) | Stmt::Print(_)) {
        ...
        let err_pos = Position::new(update_pos.end_line, update_pos.end_idx, update_pos.end_line, update_pos.end_idx + 1);
        parser_consume!(self, Paren(LeftBrace), &err_pos, "Expect '{' after the update statement.".to_string())?;

        let chunk = self.block_stmt()?;

        let final_pos = self.get_final_pos();

        return Ok(Stmt::For(Box::new(
            StmtFor {
                pos: Position::new(for_pos.start_line, for_pos.start_idx, final_pos.end_line, final_pos.end_idx),
                init,
                condition,
                update,
                chunk,
    ...
    fn break_stmt(&mut self) -> SyntaxResult<Stmt> {
        ...
        let (tag, err_pos) =
            if let Tag(tag_name) = &self.peek().token_type {
                ...
                (None,
                 Position::new(break_pos.end_line, break_pos.end_idx, break_pos.end_line, break_pos.end_idx + 1))
            };
    ...
    fn continue_stmt(&mut self) -> SyntaxResult<Stmt> {
        ...
        let continue_pos = Position::new(keyword_continue.line, keyword_continue.start, keyword_continue.line, keyword_continue.end);
        ...
        let (tag, err_pos) =
            if let Tag(tag_name) = &self.peek().token_type {
                ...
                (None,
                 Position::new(continue_pos.end_line, continue_pos.end_idx, continue_pos.end_line, continue_pos.end_idx + 1))
            };

        parser_consume!(self, Operator(Semicolon), &err_pos, "Expect ';' after 'continue'.".to_string())?;

        let final_pos = self.get_final_pos();

        return Ok(Stmt::Continue(Box::new(
            StmtContinue {
                pos: Position::new(continue_pos.start_line, continue_pos.start_idx, final_pos.end_line, final_pos.end_idx),
                tag,
            }
        )));
    }
