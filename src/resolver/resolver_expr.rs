...
impl Resolver {
    /// 分析二元运算表达式
    pub fn resolve_binary_expr(&mut self,
                               expr: &ExprBinary,
        ...
        match (&left_res.res_type, &right_res.res_type) {
            ...
            (Float(float), Integer(_)) => {
                ...
                if let TokenType::Keyword(TokenKeyword::And | Or) = &expr.operator.token_type {
                    ...
                    return Err(CompileError::new(&expr.pos, "Cannot use operator '%' on a floating-point number.".to_string()));
                } else {
                    ...
                    match &expr.operator.token_type {
                        Operator(ope) => {
                            ...
                            if let Plus = ope {
                                ...
                                return Err(CompileError::new(&expr.pos, format!("Cannot use operator '{}' between strings.", Self::operator_to_string(&expr.operator))));
                            }
                        }
                        _ => return Err(CompileError::new(&expr.pos, format!("Cannot use operator '{}' between strings.", Self::operator_to_string(&expr.operator))))
                    }
                } else {
                    return Err(CompileError::new(&expr.pos, "Cannot operate on two objects.".to_string()))
                }
            }
            // 其余组合均无效
    ...
    /// 分析分组表达式
    pub fn resolve_grouping_expr(&mut self,
                                 inside_expr_res: &ExprResolveRes) -> CompileResult<ExprResolveRes> {
    ...
    /// 分析单元运算表达式
    pub fn resolve_unary_expr(&mut self,
                              expr: &ExprUnary,
                          right_res: &ExprResolveRes) -> CompileResult<ExprResolveRes> {
        let res_type;
        let ope_type;
        match &right_res.res_type {
            ...
            Float(float) => {
                if let TokenType::Keyword(TokenKeyword::Not) | TokenType::Operator(TokenOperator::Tilde) = &expr.operator.token_type {
                    return Err(CompileError::new(&expr.pos, format!("Cannot use operator '{}' on a floating-point number.", Self::operator_to_string(&expr.operator))));
                } else {
                    ope_type = Float(float.clone())
                }
            }
            ...
            Bool => {
                if let TokenType::Keyword(TokenKeyword::Not) = &expr.operator.token_type {
                    ...
                    return Err(CompileError::new(&expr.pos, format!("Cannot use operator '{}' on a bool.", Self::operator_to_string(&expr.operator))));
                }
            }
            // 其他类型均无效
    ...
    /// 分析类型转换表达式
    pub fn resolve_as_expr(&mut self,
                           expr: &ExprAs,
                           inside_expr_res: &ExprResolveRes) -> CompileResult<ExprResolveRes> {
