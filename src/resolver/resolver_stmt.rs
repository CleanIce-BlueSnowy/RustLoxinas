...
impl Resolver {
    ...
    /// 分析变量定义语句
    pub fn resolve_let_stmt(
        &mut self,
        stmt: &StmtLet,
        init_expr_res: Option<&ExprResolveRes>,
    ) -> CompileResult<(ValueType, usize)> {
        // 定义变量，赋值以避免多次可变引用
            ...
            if let Some(ty) = &variable.var_type {
                if !Self::check_type_convert(&expr_res.res_type, ty) {
                    ...
                    return Err(CompileError::new(
                        pos,
                        format!("Cannot convert '{}' to '{}'", expr_res.res_type, ty),
                    ));
                }
            } else {
        ...
        if let (None, None) = (&variable.var_type, init_expr_res) {
            return Err(CompileError::new(
                &stmt.let_pos,
                "Must provide at least one of the type identifier and initialization expression."
                    .to_string(),
            ));
        }
        ...
        if stmt.is_ref {
            if let Some(expr) = &stmt.init {
                if !Self::check_left_value(expr) {
                    return Err(CompileError::new(
                        &expr_get_pos!(expr),
                        "Expect a left value expression.".to_string(),
                    ));
                }
            }
        }
    ...
    /// 分析变量延迟初始化语句
    pub fn resolve_init_stmt(
        &mut self,
        stmt: &StmtInit,
        init_expr_res: &ExprResolveRes,
        in_loop: bool,
    ) -> CompileResult<(ValueType, usize, Option<usize>)> {
        ...
        if in_loop {
            if let None = self.find_variable_in_current_scope(&stmt.name) {
                return Err(CompileError::new(
                    &stmt.name_pos,
                    "Cannot initialize an externally scoped variable in a loop.".to_string(),
                ));
            }
        }
        ...
        if !variable.defined {
            return Err(CompileError::new(
                &stmt.name_pos,
                "Initialize a variable before it's defined.".to_string(),
            ));
        }
        ...
        if variable.initialized {
            return Err(CompileError::new(
                &stmt.name_pos,
                "Variable has already been initialized.".to_string(),
            ));
        }
        ...
        let right_slot = if variable.is_ref {
            if let Expr::Variable(var) = &stmt.init {
                ...
                return Err(CompileError::new(
                    &expr_get_pos!(&stmt.init),
                    "Expect a lvalue.".to_string(),
                ));
            }
        } else {
            None
        ...
        // 检查类型转换
        if !Self::check_type_convert(
            &init_expr_res.res_type,
            &variable.var_type.as_ref().unwrap(),
        ) {
            return Err(CompileError::new(
                &expr_get_pos!(&stmt.init),
                format!(
    ...
    pub fn resolve_assign_stmt(
        ...
        for idx in 0..vars_res.len() {
            ...
            if !Self::check_left_value(var) {
                return Err(CompileError::new(
                    &expr_get_pos!(var),
                    "Except a left value expression.".to_string(),
                ));
            }
            if !Self::check_type_convert(&right_res.res_type, &var_res.res_type) {
                return Err(CompileError::new(
                    ...
                    format!(
                        "Cannot convert {} to {}",
                        right_res.res_type, var_res.res_type
                    ),
                ));
            }
        }
    ...
    pub fn resolve_if_stmt(
        ...
        else_if_expr_res: &[ExprResolveRes],
    ) -> Result<(), Vec<CompileError>> {
        let mut errors = vec![];

        if !matches!(if_expr_res.res_type, ValueType::Bool) {
            errors.push(CompileError::new(
                &expr_get_pos!(&stmt.if_branch.0),
                "The expression must return a bool value.".to_string(),
            ));
        }
    ...
    pub fn resolve_while_stmt(
        &mut self,
        stmt: &StmtWhile,
        condition_res: &ExprResolveRes,
    ) -> CompileResult<()> {
        if !matches!(condition_res.res_type, ValueType::Bool) {
            Err(CompileError::new(
                &expr_get_pos!(&stmt.condition),
                "The expression must return a bool value.".to_string(),
            ))
        } else {
            Ok(())
