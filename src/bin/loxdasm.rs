...
fn main() {
    ...
    if "--help" == &args[1] {
        ...
        println!(
            "        If you don't set this argument, the result will be printed to the console."
        );
        return;
    }
    ...
    while i < args.len() {
        ...
        match arg {
            "-o" | "--output" => {
                ...
                file = Some(
                    match File::create(if let None = file_path.extension() {
                        format!("{}.loxa", &args[i])
                    } else {
                        args[i].clone()
                    }) {
    ...
    if let Err(err) = disassemble_file(
        source_path,
